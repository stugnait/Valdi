...
export default function ClientsPage() {
  ...
  const filteredClients = clients.filter((client) => {
    ...
    const matchesStatus = statusFilter === "active_only" ? client.status !== "archived" : client.status === "archived"
    return matchesSearch && matchesCountry && matchesStatus
  })

  const mapApiClient = (apiClient: ApiClient): Client => ({
    id: apiClient.id.toString(), name: apiClient.name, companyName: apiClient.company_name || undefined, contactPerson: apiClient.contact_person || undefined,
    ...
    notes: apiClient.notes || undefined, status: apiClient.status, createdAt: apiClient.created_at, totalRevenue: Number(apiClient.total_revenue || 0), activeProjects: apiClient.active_projects ?? 0,
  })
